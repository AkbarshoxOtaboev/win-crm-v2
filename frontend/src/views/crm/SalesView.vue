  ...
  <AdminLayout>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <div class="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-800">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400">Sale order lar</p>
        </div>
            ...
            <tr v-for="o in filtered" :key="o.id" class="border-b border-gray-100 dark:border-gray-800">
              ...
              <td class="td text-right">
                <div class="inline-flex items-center gap-1.5 justify-end">
                  ...
                  <RowActions :edit="false" @delete="onDelete(o)" />
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-99999 flex items-center justify-center bg-black/40 p-4" @click.self="modalOpen = false">
      <div class="w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        <h3 class="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">Yangi savdo</h3>
...
function toApiDate(local: string) {
  // datetime-local → LocalDateTime-ish
...
function openCreate() {
  ...
  form.orderDate = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`
  form.totalSum = 0
  form.comment = ''
...
async function onStatus(o: SaleOrder, status: string) {
  try {
    ...
    error.value = e instanceof ApiError ? e.message : 'Holat o‘zgartirishda xatolik'
  }
}
